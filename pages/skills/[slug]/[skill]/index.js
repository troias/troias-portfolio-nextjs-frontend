        ...
        <div className="max-w-screen-lg  w-full">
          <div className="pr-2 pl-2 flex-col sm:flex justify-around">
            <div className="w-64 h-64 m-auto ">
              {skill.attributes.cover.data && (
                <Image media={skill.attributes.cover} />
              )}
            </div>
            <div className="w-4/5 sm:w-2/5 flex m-auto mb-8">
              <p>{skill.attributes.body}</p>
            </div>
          </div>
          ...
          <div className="flex justify-center mt-6 mb-8">
            {skill.attributes.link &&
              skill.attributes.link.map((skill) => (
                <div key={skill.id}>
                  {console.log("skillurl", skill.url)}
                  <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mr-4 hover:bg-sky-700 ">
          ...
          {skill.attributes.sandbox_url && (
            <div className="mt-6 pr-2 pl-2 mb-6">
...
export const getStaticPaths = async (ctx) => {
  ...
  const paths = getSlugsJson.data.skillCategories.data
    .map((skillCategory) => {
